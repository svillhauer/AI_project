...
class LoopReader:
    ...
    def create(self,
        ...
        if isinstance(theGroups, int):
            theGroups = [[i/theGroups, (i+1)/theGroups] for i in range(theGroups)]
        theGroups[-1][-1] = 2

        print('LOADING OVERLAP MATRIX. PLEASE BE PATIENT.')
        overlap_path = os.path.join(basePath, 'OVERLAP.csv')
        overlapMatrix = np.loadtxt(overlap_path, delimiter=',') / 100
        ...
        for curGroup in theGroups:
            ...
            np.random.shuffle(curLoop.T)
            self.loopSpecs.append(curLoop.astype(int))

        if doBalance:
            print('BALANCING DATA.')
            minSize = min(curLoop.shape[1] for curLoop in self.loopSpecs)
            self.loopSpecs = [spec[:, :minSize] for spec in self.loopSpecs]

        if outContinuous:
            print('PREPARING CONTINUOUS OUTPUT.')
            self.loopSpecs = [
                np.vstack((spec, (1 - overlapMatrix[spec[0], spec[1]]) if doInvert else overlapMatrix[spec[0], spec[1]]))
                for spec in self.loopSpecs
            ]
        else:
            print('PREPARING DISCRETE OUTPUT.')
            for i in range(len(self.loopSpecs)):
                curTag = len(self.loopSpecs) - i - 1 if doInvert else i
                theTags = np.full((1, self.loopSpecs[i].shape[1]), curTag)

                if toCategorical:
                    theTags = to_categorical(theTags.T, len(self.loopSpecs)).T
        ...
        self.loopSpecs = np.concatenate(self.loopSpecs, axis=1)
        ...
        np.random.shuffle(self.loopSpecs.T)
        print('DONE!')
    ...
    def save(self, baseName, forceOverwrite=False):
        if not forceOverwrite and self.is_saved(baseName):
            print(f'[ABORTING] File {baseName}.pkl already exists.')
            return
        with open(baseName + '.pkl', 'wb') as f:
            dump([self.loopSpecs, self.creationParameters], f)

    def load(self, baseName):
        if not self.is_saved(baseName):
            print(f'[ABORTING] Reader file {baseName}.pkl not found.')
            return
        with open(baseName + '.pkl', 'rb') as f:
            [self.loopSpecs, self.creationParameters] = load(f)
...
#         print('LOADING OVERLAP MATRIX. PLEASE BE PATIENT.')
#         overlapmatrix_directory = "/home/svillhauer/Desktop/AI_project/UCAMGEN-main/REALDATASET/"

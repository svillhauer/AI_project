...
#               truth of a UCAMGEN dataset and the user-provided hit/failure 
#               ratios and noise levels to simulate image matching.
# Author      : Antoni Burguera (antoni dot burguera at uib dot com)
# History     : Updated for resizing and RGB consistency.
...
class DataSimulator:
    ...
    FNAME_RELPOSO = 'IMGRPOSO.pkl'   # Relative motions in Orientation
    FNAME_ODOM = 'ODOM.pkl'          # Ground truth odometry

    ###########################################################################
    # CONSTRUCTOR
    ###########################################################################
    def __init__(self, dataPath, loadImages=False, minOverlap=50, motionSigma=5, 
                 angleSigma=2 * np.pi / 180, TN=0.99, FP=0.01, FN=0.01, TP=0.99,
        ...
        self.img_height, self.img_width, self.img_channels = target_shape
        self.odoCovariance = np.array([[motionSigma ** 2, 0, 0], 
                                        [0, motionSigma ** 2, 0], 
                                        [0, 0, angleSigma ** 2]])
        ...
        self.xMin, self.xMax = np.min(z), np.max(z)
        (_, _, z) = find(self.relPosY)
        self.yMin, self.yMax = np.min(z), np.max(z)
        (_, _, z) = find(self.relPosO)
        self.oMin, self.oMax = np.min(z), np.max(z)
    ...
    def update(self):
        self.curImage = None
        if self.loadImages:
            fileName = os.path.join(self.dataPath, 'IMAGES', f'IMAGE{self.curStep + 1:05d}.png')
            image = imread(fileName)

            # Resize and ensure RGB format
            image = resize(image, (self.img_height, self.img_width), anti_aliasing=True)
            if image.ndim == 2:  # Convert grayscale to RGB if needed
                image = gray2rgb(image)
            self.curImage = img_as_float(image)
    ...
    def match_images(self, firstID, secondID, addNoise=True, simulateFailures=False):
        ...
        if simulateFailures:
            ...
            if doMatch:
                ...
                if matchDetected:
                    theMotion = np.array([self.relPosX[firstID, secondID], 
                                          self.relPosY[firstID, secondID], 
                                          self.relPosO[firstID, secondID]])
            else:
                matchDetected = randomNumber > self.trueNegativeRate
                if matchDetected:
                    theMotion = np.array([np.random.uniform(self.xMin, self.xMax),
                                          np.random.uniform(self.yMin, self.yMax),
                                          np.random.uniform(self.oMin, self.oMax)])
        else:
            theMotion = np.array([self.relPosX[firstID, secondID], 
                                  self.relPosY[firstID, secondID], 
                                  self.relPosO[firstID, secondID]])
